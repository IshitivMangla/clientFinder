...
/* ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ∩┐╜  // Store site data by lead ID ΓÇö avoids inline string escaping bugs
  window._siteData = {};
  rows.forEach(l => {
  ...
  tbody.innerHTML = rows.map(l => {
    ...
    const STATUS = {
      pending:          ['b-pend',  'ΓÅ│ Pending'],
      discovered:       ['b-pend',  '≡ƒöì Discovered'],
      pending_outreach: ['b-ready', '≡ƒô¼ Ready to Send'],
      outreached:       ['b-out',   '≡ƒôº Outreached'],
      engaged:          ['b-eng',   'Γ£à Engaged'],
      no_email_found:   ['b-none',  'Γ¥ô No Email'],
      has_website:      ['b-hasweb','≡ƒîÉ Has Site'],
      skipped_type:     ['b-none',  'ΓÇö Skipped'],
      skipped_website:  ['b-none',  'ΓÇö Skipped'],
    };
    ...
    const emailCell = (l.email && l.email.trim())
      ...
      : `<span style="color:var(--t3);font-size:.72rem;display:flex;align-items:center;gap:4px"><i class="fa-solid fa-rotate fa-spin" style="font-size:.6rem"></i> ScrapingΓÇª</span>`;
    ...
    // Use data-id ΓÇö no inline string escaping; site data stored in window._siteData
    ...
    return `<tr class="${rc}">
      <td><div class="ln">${hEsc(l.name||'Unknown')}</div><div class="la"><i class="fa-solid fa-location-dot"></i>${hEsc(l.address||'ΓÇö')}</div></td>
      <td>${typeBadge}</td>
      <td>${emailCell}</td>
      <td>${webCell}</td>
      <td>${statusBadge}</td>
  ...
  }).join('');,   s.outreached ?? 'ΓÇö');
    setText('k-eng',   s.engaged ?? 'ΓÇö');
    setText('k-pend',  s.pending_count ?? 'ΓÇö');
...
function renderLeads() {
  ...
  tbody.innerHTML = rows.map(l => {
    ...
    const STATUS = {
      pending:          ['b-pend',  'ΓÅ│ Pending'],
      discovered:       ['b-pend',  '≡ƒöì Discovered'],
      pending_outreach: ['b-ready', '≡ƒô¼ Ready'],
      outreached:       ['b-out',   '≡ƒôº Outreached'],
      engaged:          ['b-eng',   'Γ£à Engaged'],
      no_email_found:   ['b-none',  'Γ¥ô No Email'],
      has_website:      ['b-hasweb','≡ƒîÉ Has Site'],
    ...
    return `<tr>
      <td><div class="ln">${l.name||'Unknown'}</div><div class="la"><i class="fa-solid fa-location-dot"></i>${l.address||'ΓÇö'}</div></td>
      <td>${typeBadge}</td>
      <td>${emailCell}</td>
      <td>${webBadge}</td>
      <td>${statusBadge}</td>
...
async function sendEmail(id, btn) {
  ...
  toast('Sending outreach emailΓÇª', 'info');
  try {
...
function updateBtns(tasks) {
  updBtn('find-leads',    tasks.find_leads,    'Finding LeadsΓÇª',    'Discover New Leads Now',  'fa-magnifying-glass');
  updBtn('check-replies', tasks.check_replies, 'Checking RepliesΓÇª', 'Check Email Replies',     'fa-envelope');
  updBtn('outreach',      tasks.outreach,      'Sending EmailsΓÇª',   'Send Batch Outreach',     'fa-paper-plane');
}
...
async function trigger(name) {
  ...
  toast(`Starting ${name.replace('-',' ')}ΓÇª`, 'info');
  try {
